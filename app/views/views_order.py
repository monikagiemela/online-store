...
from app.models import Cart, Casecolors, Content, Contentcolors, Invoices,  Orders, Products, Postage, Realization, Users, Payment
    ...
    # USER SENT AN ORDER FORM
        ...
        if request.form.get('front-end-data'):
            ...
            try: 
                ...
                # User wants to register when making an order  
                ...
                if request.form.get('user-password'):                    
                    ...
                    if company_name:
                        user = Users(first_name=user_name, 
                ...
                # Register an order in Orders tabel iwth user_id
            ...
            if company_name and nip:
                invoice = 'yes'
                invoice_ = Invoices(order_id=order_id, 
                                    company=company_name, 
                                    nip=nip, 
                                    address=postage_address, 
                                    post_code=user_postcode, 
            ...
            for content_id, product_ids in cart_dictionary.items(): 
                order_product = Cart(product_id=product_ids[0], 
                                    ...
                                    content_color_id=product_ids[1], 
                                    case_color_id=product_ids[2], 
                                    price=product_ids[3], 
                                    order_id=order_id)
            ...
            #message = Message("Dziękujemy za zamówienie!", recipients=[user_email])
            #mail.send(message)
            # Send email to the store administrator
            message = Message("Masz nowe zamówienie!", recipients=[user_email])
            message.html = render_template("order_mail.html", 
                                            user_name=user_name, 
                                            user_lastname=user_lastname, 
                                            user_phone=user_phone, 
                                            postage_address=postage_address, 
                                            ...
                                            nip=nip, 
                                            order_id=order_id, 
                                            extra_postage_info=extra_postage_info,
                                            postage_price=postage_price,
                                            transaction_total=transaction_total, 
                                            cart_value=cart_value)
            mail.send(message)
            flash(f"Dziękujemy za zamówienie! Potwierdzenie zamówienia zostało wysłane na adres {user_email}")    
            return redirect("/")    
        
        # USER USED A LOG IN MODAL IN THIS TEMPLATE TO LOG IN  
